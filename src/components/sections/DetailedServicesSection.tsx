...
const DetailedServicesSection = () => {
  ...
  return <section id="detailed-services" className="py-24 bg-muted/20">
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-7xl mx-auto">
          {services.map((service, index) => <Card key={index} className={`border-0 shadow-card hover:shadow-elegant transition-all duration-300 bg-gradient-card group ${service.isPlugPlay ? 'ring-2 ring-brand-red/30 bg-gradient-to-br from-brand-red/5 to-white' : ''}`}>
              <CardHeader className="pb-4">
                <div className="flex items-start space-x-4">
                  <div className={`w-12 h-12 ${service.isPlugPlay ? 'bg-brand-red/10 group-hover:bg-brand-red/20' : 'bg-brand-red/10 group-hover:bg-brand-red/20'} rounded-xl flex items-center justify-center flex-shrink-0 transition-all duration-300`}>
                    <service.icon className={`w-6 h-6 ${service.isPlugPlay ? 'text-brand-red' : 'text-brand-red'}`} />
                  </div>
                  <div className="flex-1">
                    <CardTitle className={`text-lg font-bold leading-tight ${service.isPlugPlay ? 'text-brand-red' : 'text-foreground'}`}>
                      {service.title}
                      {service.isPlugPlay && <span className="ml-2 text-xs bg-brand-red text-white px-2 py-1 rounded-full font-normal">
                          NUOVO
                        </span>}
                    </CardTitle>
                  </div>
              ...
              <CardContent className="pt-0">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {service.features.map((feature, idx) => <div key={idx} className="flex items-center text-sm text-muted-foreground">
                      <div className={`w-2 h-2 rounded-full ${service.isPlugPlay ? 'bg-brand-red' : 'bg-brand-red'} mr-2 flex-shrink-0`}></div>
